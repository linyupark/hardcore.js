...
riot.tag2('project-history', '<section><h2> 管理后台 &gt; {app.lang.admin.project.title} &gt; 项目执行历史 </h2><table class="base"><thead><tr><th width="10%">序号</th><th width="10%">项目编号</th><th width="50%">名称</th><th width="20%">项目起止日期</th><th width="10%">{app.lang.admin.handle}</th></tr></thead><tbody><tr each="{tableList}"><td>{id}</td><td>{project_number}</td><td>{project_name}</td><td>{app.utils.time2str(created_at)} ~ {finished_at?app.utils.time2str(finished_at):\'未结束\'}</td><td><a href="#!admin-project/view/{parent.pid}?id={id}" class="under-line">查看详情</a></td></tr><tr if="{!tableList}"><td colspan="5"><spinner-dot></spinner-dot></td></tr></tbody><tfoot if="{tableList}"><tr><td class="left" colspan="5"> {app.lang.admin.counts.items} <b>{items}</b> {app.lang.admin.counts.unit} </td></tr></tfoot></table><pagination-number page="{page}" pages="{pages}" select="y"></pagination-number></section>', '', '', function(opts) {
  var _this = this;
...
riot.tag2('project-view', '<section><h2> 管理后台 &gt; {app.lang.admin.project.title} &gt; 项目详细 </h2><div class="project-view"><span class="header-mark"> 项目起止日期: {project.perform.created_at?app.utils.time2str(project.perform.created_at):\'未开始\'} ~ {project.perform.finished_at?app.utils.time2str(project.perform.finished_at):\'未结束\'} </span><h4>基本信息</h4><div class="c2"><div class="row"><p><label>项目名称:</label> {project[\'base-info\'].name} </p><p><label>项目编号:</label> {project[\'base-info\'].number} </p></div><div class="row"><p><label>项目类型:</label> {project[\'base-info\'].project_types_name} </p><p><label>项目状态:</label><span class="txt-success">{app.getProjectStatus(project[\'base-info\'].status)}</span></p></div><div class="row"><p><label>项目总金额:</label> {project[\'base-info\'].amount} </p><p><label>公开项目:</label> {project[\'base-info\'].is_public==1?\'是\':\'否\'} </p></div></div><div class="c1"><label>项目简介:</label><p> {project[\'base-info\'].description} </p></div><br><hr><h4>执行信息</h4><div class="c1"><label>执行单位:</label><p> {project.performer.organization_name} </p></div><div class="c2"><div class="row"><p><label>单位负责人:</label> {project.performer.head_name} </p><p><label>联系电话:</label> {project.performer.head_tel} </p></div><div class="row"><p><label>项目联络人:</label> {project.performer.contact_name} </p><p><label>联系电话:</label> {project.performer.contact_tel} </p></div></div><br><hr><div each="{ag, i in project.agreement}"><h4>项目关联协议({i+1})</h4><div class="c2"><div class="row"><p><label>协议名称:</label> {ag.agreement_name} </p><p><label>协议编号:</label> {ag.agreement_number} </p></div><div class="row"><p><label>关联时间:</label> {app.utils.time2str(ag.create_date)} </p></div></div><br><hr></div><h4>到款状态</h4><div class="c1"><label>是否已到款:</label><p> {project.payment_status?\'是\':\'否\'} </p></div><br><hr><h4>项目执行</h4><div class="row-step"><div class="step-box"><div class="step-num {active:project.perform.create_status==1}">1</div><div class="step-line"></div><div class="step-num {active:project.perform.payment_status==1}">2</div><div class="step-line"></div><div class="step-num {active:project.perform.review_status==1}">3</div><div class="step-line"></div><div class="step-num {active:project.perform.verify_status==1}">4</div><div class="step-line"></div><div class="step-num {active:project.perform.confirm_status==1}">5</div><div class="step-line"></div><div class="step-num {active:project.perform.issue_status==1}">6</div></div><div class="text-box"><p>立项</p><p>确认到款</p><p>录入评审结果</p><p>项目部审核</p><p>捐赠方确认</p><p>发放完成</p></div></div><br><hr><div class="c1 btn-line" style="text-indent: 0; text-align: center"><button type="button" onclick="{fn.back}" class="btn-gray">{app.lang.admin.btn.back}</button></div></div></section>', '', '', function(opts) {
  var _this= this;
  _this.pid = _this.app.route.params[2];
...
riot.tag2('project-agreement', '<div class="project-agreement" if="{opts.pid>0}"><table-filter for="project-agreement"><yield to="addon"><button type="button" class="main" onclick="{parent.fn.modalAddAgreement}"><i class="icon-plus"></i> 添加关联协议 </button></yield></table-filter><br><table class="base"><thead><tr><th width="10%">{app.lang.admin.agreement.id}</th><th width="15%">{app.lang.admin.project.number}</th><th width="50%">{app.lang.admin.agreement.name}</th><th width="15%">关联日期</th><th width="10%">{app.lang.admin.handle}</th></tr></thead><tbody><tr each="{tableList}"><td>{id}</td><td>{agreement_number}</td><td class="left">{agreement_name}</td><td>{app.utils.time2str(create_date)}</td><td><a href="javascript:;" aria-label="取消关联" class="c-tooltip--top"><i onclick="{fn.remove}" class="btn-icon icon-trash"></i></a></td></tr><tr if="{!tableList}"><td colspan="5"><spinner-dot></spinner-dot></td></tr></tbody><tfoot if="{tableList}"><tr><td class="left" colspan="5"> {app.lang.admin.counts.items} <b>{items}</b> {app.lang.admin.counts.unit} </td></tr></tfoot></table><pagination-number page="{page}" pages="{pages}" select="y"></pagination-number><br><br><br></div><div class="warning-box" if="{opts.pid==0}"> 需要先创建项目后才能编辑此页 <a href="#!{app.route.path}?tab=baseinfo">返回创建</a></div><modal-agreement pid="{opts.pid}" ref="agmodal"></modal-agreement><modal-remove ref="rmmodal"></modal-remove>', 'project-agreement .project-agreement table-filter div,[data-is="project-agreement"] .project-agreement table-filter div{ margin: 0 }', '', function(opts) {
  var _this = this;
  _this.fn = {
    remove: function(e){
