...
riot.tag2('fp-admin-user', '<header for="admin"></header><main class="admin"><div class="container"><admin-sidenav></admin-sidenav><user-form if="{section==\'add\'}"></user-form><user-form if="{section==\'edit\'}"></user-form><section if="{section==\'index\'}"><h2>管理后台 &gt; 用户管理</h2><table-filter for="user"><yield to="addon"><button class="main" onclick="{parent.fn.add}"><i class="icon-plus"></i> 添加用户 </button></yield></table-filter><table class="base"><thead><tr><th width="10%">序号</th><th width="15%">姓名</th><th width="10%">性别</th><th width="20%">系统角色</th><th width="10%">用户类型</th><th width="15%">手机号</th><th width="10%">注册时间</th><th width="10%">{app.lang.admin.handle}</th></tr></thead><tbody><tr each="{tableList}"><td>{id}</td><td>{real_name}</td><td>{sex}</td><td>{role_name||\'-\'}</td><td>{is_manager==1?\'管理员\':\'非管理员\'}</td><td>{tel||\'-\'}</td><td>{app.utils.time2str(created_at)}</td><td><a href="javascript:;" aria-label="{app.lang.admin.handles.edit}" class="c-tooltip--top"><i onclick="{fn.edit}" class="btn-icon icon-pencil"></i></a><a href="javascript:;" aria-label="{app.lang.admin.handles.remove}" class="c-tooltip--top"><i onclick="{fn.remove}" class="btn-icon icon-trash"></i></a></td></tr><tr if="{!tableList}"><td colspan="8"><spinner-dot></spinner-dot></td></tr></tbody><tfoot if="{tableList}"><tr><td class="left" colspan="8"> {app.lang.admin.counts.items} <b>{items}</b> {app.lang.admin.counts.unit} </td></tr></tfoot></table><pagination-number page="{page}" pages="{pages}" select="y"></pagination-number></section></div></main><footer class="admin"></footer><modal-remove></modal-remove>', '', '', function(opts) {
  var _this = this;
  ...
  _this.fn = {
    remove: function(e){
      ...
      .emit('open').once('ok', function(){
        _this.app.api('GET', 'user-manager/default/delete', {
          data: { id: e.item.id }
        }).on('done', function(){
          _this.app.alert('用户删除成功', 'success');
